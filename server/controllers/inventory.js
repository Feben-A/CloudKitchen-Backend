...
class InventoryController {
  ...
  static async createOrUpdateInventory(req, res) {
    try {
      ...
      const existingIngredient = await Inventory.getByNameAndRestaurant(
        name,
        restaurant_id
      );
      ...
      if (existingIngredient) {
        ...
        const updatedItem = await Inventory.increaseStock(
          name,
          category,
          quantity,
          unit,
  ...
  static async deductIngredientStock(req, res) {
    try {
      ...
      const updatedStock = await Inventory.deductStockFIFO(
        name,
        quantity,
        restaurant_id
      );
  ...
  static async removeExpiredStock(req, res) {
    try {
      ...
      res
        .status(200)
        .json({ message: `${removedCount} expired items removed` });
    } catch (error) {
