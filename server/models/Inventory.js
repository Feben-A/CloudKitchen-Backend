...
class Inventory {
    ...
    static async getAll() {
        const result = await db.query("SELECT * FROM Inventory ORDER BY ingredient_id ASC");
        return result.rows;
    }
    ...
    static async create(name, category, quantity, unit, price_per_unit, expiry_date, restaurant_id) {
        const result = await db.query(
            ...
            [name, category, quantity, unit, price_per_unit, expiry_date, restaurant_id]
        );
        return result.rows[0];
    }
    ...
    static async increaseStock(name, category, quantity, unit, price_per_unit, expiry_date, restaurant_id) {
        ...
        if (existingIngredient.rows.length > 0) {
            ...
            return await this.create(name, category, quantity, unit, price_per_unit, expiry_date, restaurant_id);
        }
    }
    ...
    static async deductStock(name, restaurant_id, quantityToDeduct) {
        ...
        for (let batch of batches.rows) {
            ...
            if (batch.quantity - deductAmount <= 0) {
                await db.query("DELETE FROM Inventory WHERE ingredient_id = $1", [batch.ingredient_id]);
            } else {
                await db.query(
